    ...
    if qc_tracker:
        ...
        if metadata["n_spots_in_tissue"] == 0:
            ...
            qc_tracker.add_record(patient_id, "tumor_fraction", "OK",
                                  "Tumor fraction attached successfully", metadata)
...
def compute_tile_purity(
    ...
    mean_tile_purity_threshold = 0.3
    if metadata['mean_tile_purtiy'] < mean_tile_purity_threshold:
        msg = f"Mean tile purity is very low: {metadata['mean_tile_purtiy']}"
        logger.warning(msg)
        if qc_tracker:
            qc_tracker.add_record(patient_id, "tile_purity", "FLAG", msg)
    else:
        if qc_tracker:
            qc_tracker.add_record(patient_id, "tile_purity", "OK", 
                                  f"Mean tile purity is above {mean_tile_purity_threshold * 100}%")    
...
def filter_tumor_tiles(df, 
    ...
    if metadata['n_tumor_tiles'] == 0:
        ...
        logger.error(msg)
        if qc_tracker:
            qc_tracker.add_record(patient_id, "filter_tumor_tiles", "EXLUDE", msg)
        
    elif metadata['n_tumor_tiles'] < 10:
        msg = f"Only {len(tumor_tiles)} tumor tiles - insufficient for MIL training"
        logger.warning(msg)
        if qc_tracker:
            qc_tracker.add_record(patient_id, "filter_tumor_tiles", "FLAG", msg)
