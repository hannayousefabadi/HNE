...
def add_tile_coordinates(scales, 
    ...
    # sanity check
    print(f"fullres pixel size is {fullres_pixel_size:.2f} µm/pixel")
    print(f"hires pixel size is {hires_pixel_size:.2f} µm/pixel ≈ {hires_pixel_size:.1f} µm/pixel")
    ...
    print(f"# of initial tiles: {len(merged['tile_id'].unique())}")
...
def filter_tumor_tiles(df, tumor_threshold=0.3, # tile at least has 30% tumor purity
    ...
    # sanity check
    logger.info(f"# of tiles after filtering to {tumor_threshold * 100}% tumor purity: {len(tumor_tiles)}")
