...
def preprocess_patient(patient_id, 
    ...
    tiles, meta = crop_and_save_tiles(tumor_tiles, tile_size, img, patient_id, logger)
    patient_metadata.update(meta)
    ...
    patient_metadata.update(meta)
    tiles_sig_tumor, meta = aggregate_signatures(spots_df, sig_cols, tile_size, tiles, logger)
    patient_metadata.update(meta)
