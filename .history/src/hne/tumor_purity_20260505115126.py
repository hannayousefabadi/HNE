...
def filter_tumor_tiles(df, 
    ...
    # WARNING: critical check for MIL
    if metadata['n_tumor_tiles'] == 0:
        logger.error(f"NO tumor tiles found! Check threshold={tumor_threshold}, min_spots={min_spots}")
    elif metadata['n_tumor_tiles'] < 10:
        logger.warning(f"Only {metadata['n_tumor_tiles']} tumor tiles - may be insufficient for MIL training")
