...
def preprocess_patient(patient_id, 
    ...
    sig_cols, signature_genes, spots_df, meta = compute_signatures(vis, df, logger)
    patient_metadata.update(meta)
    tiles_sig_tumor, meta = aggregate_signatures(spots_df, sig_cols, tile_size, tumor_tiles, logger)
    patient_metadata.update(meta)
