...
def preprocess_patient(patient_id, 
    ...
    merged, meta = attach_tumor_fraction(spots, vis, logger)
    ...
    tumor_tiles, meta = filter_tumor_tiles(final_df, tumor_threshold, min_spots, logger)
    patient_metadata.update(meta)


    if len(tumor_tiles) == 0:
        logger.warning(f"No tumor tiles found for {patient_id}! Skipping...")
        return patient_metadata, None

    # 7. Generate & save image tiles (H&E crops)
    tumor_tiles = crop_and_save_tiles(tumor_tiles, TILE_SIZE, img, logger)
