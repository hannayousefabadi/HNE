...
from hne.data_io import load_visium, load_spots, load_scale_factor, load_he_image, save_tile_features
...
tumor_tiles = crop_and_save_tiles(tumor_tiles, tile_size=TILE_SIZE, hne_img=img)

# 8. Compute pathway signatures in spot space
sig_cols, spots_df = compute_signatures(vis=vis, df=df)

# 9. Aggregate spot signatures to tile-level
tiles_sig_tumor = aggregate_signatures(spots_df=spots_df, sig_cols=sig_cols, tile_size=TILE_SIZE, tumor_tiles=tumor_tiles)

# 10. Apply z‑score & binary calls
tiles_sig_tumor = zscore_and_binary(sig_cols=sig_cols, tiles_sig_tumor=tiles_sig_tumor).copy()
