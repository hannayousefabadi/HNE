...
if __name__ == "__main__":
    ...
    # Tile-level cohort QC
    if all_tiles_sig:
        sig_cols = [col for col in all_tiles_sig[0].columns 
                   if '_binary' not in col and '_z' not in col 
                   and col not in ['tile_id', 'tile_row', 'tile_col', 'tile_purity', 
                                   'n_spots', 'x_min_hires', 'y_min_hires', 
                                   'x_max_hires', 'y_max_hires']]
        qc.save_cohort_tile_qc_plots(all_tiles_sig, sig_cols)
